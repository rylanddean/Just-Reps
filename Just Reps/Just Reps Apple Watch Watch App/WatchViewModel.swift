...
final class WatchViewModel {
    ...
    func applyPhoneContext(
        entries: [WorkoutEntry],
        exercises: [ExerciseType],
        goals: [String: Int],
        minimumViableReps: [String: Int] = [:],
        mvrEffectiveDate: Date? = nil,
        walkingStepsToday: Int = 0
    ) {
        ...
        self.mvrEffectiveDate = mvrEffectiveDate
        self.walkingStepsToday = walkingStepsToday
    ...
    func totalReps(for exercise: ExerciseType) -> Int {
        if case .walking = exercise {
            return walkingStepsToday
        }
    ...
    var last7DaysActivity: [(date: Date, reps: Int, isFreeze: Bool, isRest: Bool)] {
        ...
        let restKeys: Set<DateComponents> = Set(
            allEntries
                .filter { $0.kind == .rest }
                .map { StreakEngine.logicalDay(for: $0.timestamp) }
        )
        return (0..<7).reversed().compactMap { offset in
            ...
            return (
                date: date,
                reps: heatmap[key] ?? 0,
                isFreeze: freezeKeys.contains(key),
                isRest: restKeys.contains(key)
            )
        }
    }
